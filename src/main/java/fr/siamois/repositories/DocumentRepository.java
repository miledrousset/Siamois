...
public interface DocumentRepository extends CrudRepository<Document, Integer> {
    ...
    @Query(
            nativeQuery = true,
            value = "SELECT * FROM siamois_document s JOIN action_unit_document aud on s.document_id = aud.fk_document_id WHERE aud.fk_action_unit_id = :actionUnit"
    )
    List<Document> findAllDocumentsOfActionUnit(@Param("actionUnit") ActionUnit actionUnit);

    @Query(
            nativeQuery = true,
            value = "SELECT * FROM siamois_document s JOIN spatial_unit_document d ON s.document_id = d.fk_document_id WHERE d.fk_spatial_unit_id = :spatialUnit"
    )
    ...
    @Query(
            nativeQuery = true,
            value = "SELECT * FROM siamois_document s JOIN recording_unit_document d ON s.document_id = d.fk_document_id WHERE d.fk_recording_unit_id = :recordingUnit"
    )
