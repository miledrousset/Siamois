...
import fr.siamois.bean.home.AbstractPanel;
...
public class NewRecordingUnitPanel extends AbstractPanel {
    ...
    // View param
    private Long unitid;  // ID of the requested RU
    ...
    public NewRecordingUnitPanel(RecordingUnitService recordingUnitService,
    ...
    public void init(ActionUnit actionUnit) {
        try {
            unitid= null; // No recording unit requested so we reinit the ID
    ...
    public void init() {
        try {
            if (!FacesContext.getCurrentInstance().isPostback() && this.unitid != null) {

                log.info("Loading RU");
                reinitializeBean();
                this.recordingUnit = this.recordingUnitService.findById(this.unitid);
